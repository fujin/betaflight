...
#include "accgyro_bmi160.h"

//#define DEBUG_MPU_DATA_READY_INTERRUPT

// BMI160, Standard address 0x68
...
#define BMI160_SMPLRT_DIV      0       // 8000Hz

static void bmi160AccInit(acc_t *acc);
static void bmi160GyroInit(uint8_t lpf);

bool bmi160AccDetect(acc_t *acc)
{
    if (mpuDetectionResult.sensor != MPU_60x0) {
        return false;
    }

    acc->init = bmi160AccInit;
    acc->read = mpuAccRead;
    acc->revisionCode = (mpuDetectionResult.resolution == MPU_HALF_RESOLUTION ? 'o' : 'n'); // es/non-es variance between BMI160 sensors, half of the naze boards are mpu6000ES.

    return true;
}

bool bmi160GyroDetect(gyro_t *gyro)
{
    if (mpuDetectionResult.sensor != MPU_60x0) {
        return false;
    }
    gyro->init = bmi160GyroInit;
