...
void fastKalmanInit(fastKalman_t *filter, float q, float r, float p, float initialValue)
{
	...
	filter->x     = initialValue;   //set initial value, can be zero if unknown
	filter->lastX = initialValue;   //set intial value, can be zero if unknown
